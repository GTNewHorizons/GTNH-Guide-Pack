...
空的合成存储器，用于填充CPU结构空间，也是其他组件的合成原料。
...
合成存储器用来给[自动合成](../ae2-mechanics/autocrafting.md)系系统提供1k/4k/16k/64k/256k存储容量，CPU容量决定CPU可处理的合成任务复杂度。
...
提升<ItemLink id="appliedenergistics2:tile.BlockInterface" />的原料分发频率，适配高速处理设备（如被多个<ItemLink id="appliedenergistics2:tile.BlockMolecularAssembler" />环绕的样板供应器）。拥有并行处理单元的合成任务可以在同一时间向多个目标发配原料。
...
显示当前CPU处理的合成任务，支持<ItemLink id="appliedenergistics2:item.ToolColorApplicator" />染色。为OpenComputer提供AE自动合成系统的API。
